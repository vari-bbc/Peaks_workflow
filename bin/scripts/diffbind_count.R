...
samples <- read_tsv(samplesheet) %>%
        dplyr::mutate(SampleID=sample, 
                      ...
                      Peaks=str_glue("analysis/macs2/rm_blacklist/{sample}_macs2_narrow_peaks.rm_blacklist.narrowPeak"),
                      PeakCaller="bed",
                      Factor=sample_group,
                      out_pref=ifelse(is.na(enriched_factor), "DB", enriched_factor)) %>%
...
for(out_pref in unique(samples$out_pref)){
    message(str_glue("Making DBA for {out_pref}"))

    curr_samps <- samples %>% 
                dplyr::filter(out_pref == out_pref) %>%
                dplyr::select(-out_pref)
    ...
    saveRDS(diffbind, str_glue("{out_dir}/{out_pref}.rds"))
}
